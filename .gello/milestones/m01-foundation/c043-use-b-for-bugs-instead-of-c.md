...
- New issues (⌘I quick capture, "+ New issue" button, report-issue) allocate
  from a `b` namespace (`b001`, `b002`, …) with its own counter; tasks keep
  `c`. Filenames follow (`b001-slug.md`). Invalid b-files reserve their ids
...
- CLAUDE.md board-query globs widened to `[bc][0-9]*.md`; concept.md
  documents both namespaces. Duplicate detection (c031) is namespace-
  agnostic (full-id keyed) and covers b-ids automatically.
