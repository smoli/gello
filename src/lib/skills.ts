...
import { BOARD_YAML, CONVENTION_SNIPPET } from "./scaffold";
...
// c0133 adds the gello-concept skill — a new skill needs no version bump: it
// has no file on disk yet, so every install target sees it as "install".
// i0174 adds gello-review the same way (same reason, no bump).
...
/**
 * A skill's instructions — the body without its YAML frontmatter. i0174: the
 * companion embeds the review skill in a prompt, where `name` / `description`
 * (there for skill discovery) are noise.
 */
export function skillInstructions(skill: SkillTemplate): string {
  const match = /^---\n[\s\S]*?\n---\n+/.exec(skill.body);
  return match ? skill.body.slice(match[0].length) : skill.body;
}
...
export const CONCEPT_SKILL: SkillTemplate = {
  folder: "gello-concept",
  version: SKILL_VERSION,
  body: `---
name: gello-concept
...
// The review skill (c0166) lived in `companion/review.ts` as a prompt string
// until i0174 moved it here: the installer's copy is now the only one, so the
// human can invoke it by hand as well. The companion still embeds it in the
// review-run prompt via `skillInstructions`, so a run depends on nothing being
// installed. The `## Review` verdict format it writes is parsed by
// `src/lib/review.ts`, which the board reads too (c0170).
export const REVIEW_SKILL: SkillTemplate = {
  folder: "gello-review",
  version: SKILL_VERSION,
  body: `---
name: gello-review
...
/** Every gello-managed skill the installer ships (c032/c029/c0133/i0174). */
